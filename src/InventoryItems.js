...
const InventoryItems = () => {
    ...
    const handleDeleteProduct = async (index) => {
        ...
        try {
            const response = await fetch(`${hostaddress}/inventory/${itemId}/products/${itemDetails.products[index].serial_number}`, {
                method: 'DELETE',
            });
            if (!response.ok) {
    ...
    const getTotalCount = (status) => {
        return itemDetails.products.filter(product => product.status === status).length;
    };
        ...
        <div className="container mx-auto px-4 mt-20">
            ...
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center">
                    <div className="flex items-center justify-center border border-black rounded-full w-6 h-6 mr-2" style={{ marginTop: '-10px' }}>
                        <FontAwesomeIcon
                            icon={faArrowLeft}
                    ...
                    <h2 className="text-2xl font-bold mb-4 inline">Inventory - {itemDetails ? itemDetails.item : ''}</h2>
                    ...
                    <div className="flex justify-end mb-4">
                        ...
                        <div className="relative">
                            ...
                            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9 11l4-4h-8l4 4z"/></svg>
                            </div>
                        </div>
                    </div>
                    <div className="overflow-x-auto shadow-md sm:rounded-lg max-h-[calc(85vh-90px)] overflow-y-auto">
                        <table className="min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead className="bg-orange-400 text-white">
                                <tr>
                                    <th className="text-center py-3 px-4 uppercase font-semibold text-sm">Number</th>
                                    <th className="text-center py-3 px-4 uppercase font-semibold text-sm">Serial Number</th>
                                    <th className="text-left py-3 px-4 uppercase font-semibold text-sm">Status</th>
                                    <th className="text-center py-3 px-4 uppercase font-semibold text-sm">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="text-gray-700">
                                {itemDetails?.products && itemDetails.products.filter(product => !statusFilter || product.status === statusFilter).map((product, index) => (
                                    <tr key={index} className="bg-white border-b hover:bg-gray-50 transition ease-in-out duration-150">
                                        <td className="text-center py-3 px-4 text-blue-600 font-semibold">{index + 1}</td>
                                        <td className="text-center py-3 px-4">{product.serial_number}</td>
        ...
        <div className="space-y-2">
            <div className="flex items-center border border-gray-300 rounded p-4 mb-2">
                <div className="md:w-1/7 mr-4">
                    <img src={itemDetails.image_url} alt={itemDetails.item} className="w-full h-auto rounded-md shadow-lg" />
                </div>
                <div>
                    ...
                    <div className="flex items-center mb-2">
                        <p className="text-sm font-semibold mr-2">Brief Description:</p>
