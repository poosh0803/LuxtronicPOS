  ...
  `}
      ...
      <div className="h-full bg-gradient-to-b from-orange-500 to-orange-300 w-72 px-3 py-5 border-r-4 border-r-orange-300">
        <div>
          <div className="flex flex-row gap-5 ">
            <div>
              {" "}
              <img class="object-contain h-12 w-26" src={profile} alt="Logo" />
            </div>
          ...
          <div className="mt-10 flex flex-col gap-3">
            {tabs.map((tab, index) => (
              <Link
                key={index}
                to={tab.slug}
                className={`flex items-center p-3 gap-2 w-full rounded-md ${
                  activeTab === tab.slug
                    ? "bg-white font-semibold text-blue-500"
                    : "font-sans text-neutral-50 "
                }`}
                onClick={() => {
              ...
              >
                <img
                  ...
                  alt={`${tab.label} icon`}
                  className={`h-6 w-6 ${
                    activeTab === tab.slug ? "icon-active" : "icon"
                  }`}
                />
                {tab.label}
              </Link>
      ...
      <div className="flex-1 overflow-auto">
        <Routes>
          ...
          <Route
            path="/inventory/${itemId}/products/${itemDetails.products[index].serial_number"
          ...
          <Route
            path="/InvoiceFormCurrentClient"
            element={<InvoiceFormCurrentClient />}
          />
