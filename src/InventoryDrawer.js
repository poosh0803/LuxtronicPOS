...
const InventoryDrawer = ({ isOpen, closeDrawer, fetchInventory }) => {
    ...
    const validateForm = () => {
        ...
        if (!inventoryData.price_from_supplier || isNaN(inventoryData.price_from_supplier)) {
                ...
                {Object.entries(inventoryData).map(([key, value]) => (
                    key !== 'image_url' ? (
                        <label className="block mt-4" key={key}>
                            ...
                            <input
                                type={key === 'retail_price' || key === 'price_from_supplier' || key === 'quantity' ? 'number' : 'text'}
                                name={key}
                                value={value}
                                onChange={handleInputChange}
                                className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md"
                                placeholder={`Enter ${key.replace(/_/g, ' ').replace(/^./, str => str.toUpperCase())}`}
                            />
                        </label>
                    ) : (
                        <label className="block mt-4" key={key}>
