...
const BookingDrawer = ({ isOpen, closeDrawer, booking, fetchBookings, saveBooking }) => {
  ...
  const handleSaveClick = (e) => {
    ...
    if (validateForm()) {
      ...
      const url = booking ? `${hostaddress}/bookings/${booking.id}` : `${hostaddress}/bookings`;
  ...
  return (
    <div className={`absolute right-0 top-0 w-96 bg-white p-4 transition-transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      ...
      <form onSubmit={handleSaveClick}>
          ...
          <div key={key}>
            <label className="block mb-2 capitalize">
              ...
              {key === 'studentDiscount' || key === 'serviceType' ? (
                ...
                >
                  {key === 'studentDiscount' ? ['Yes', 'No'].map(option => <option key={option} value={option}>{option}</option>) :
                  ['Common Service', 'Inspection Service', 'PC assembling service'].map(option => <option key={option} value={option}>{option}</option>)}
                </select>
              ) : (
                <input
        ...
        <button type="submit" className="bg-orange-400 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded mt-4">
          Save
        </button>
        <button type="button" onClick={closeDrawer} className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded ml-2 mt-4">
          Close
        </button>
      </form>
    </div>
