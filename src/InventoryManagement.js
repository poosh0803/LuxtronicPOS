      ...
      .then(async (data) => {
        const filteredInventory = data.filter(
          (item) =>
            ...
            (item.price_from_supplier !== null &&
              item.price_from_supplier
                .toString()
                .toLowerCase()
        ...
        const updatedInventory = await Promise.all(
          data.map(async (item) => {
            ...
            const availableProducts = itemDetails.products.filter(
      ...
      <div className="mx-5 overflow-x-auto shadow-md sm:rounded-lg max-h-[calc(105vh-200px)]">
        <table className="min-w-full text-sm text-center text-gray-500">
          ...
          <tbody className="text-gray-700">
            {inventory.map((item) => (
              <tr
                className="hover:bg-orange-100 border-b "
                key={item.inventory_id}
              >
                <td
                  className="py-3 px-4 cursor-pointer hover:text-blue-500 hover:underline"
                  onClick={() => handleItemClick(item.inventory_id)}
                >
                  {item.item}
                </td>
                ...
                <td className="py-3 px-4">
                  ...
                  >
                    <div className="font-bold underline uppercase text-center">
                      {item.item}
                    </div>

                    <div className="py-1">
                      <span className="px-2 font-bold">Model -</span>
                      {item.model}
                    </div>
                    <div className="py-1">
                      <span className="px-2 font-bold">Brand -</span>
                      {item.brand}
                    </div>
                    <div className="py-1">
